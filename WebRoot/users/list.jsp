...
<script type="text/javascript">
	function search(){
		document.forms[0].action="userselectFirstPage";
		document.forms[0].submit();
	}
	function show(next){
		document.forms[0].action="userselectNextPage?next="+next;
		document.forms[0].submit();
	}
	function jumpto(totalPage){
		...
		document.forms[0].action="userselectNextPage?next="+num;
		document.forms[0].submit();
	}
</script>
</head>
...
<div class="page_title">用户管理 </div>
 ...
 <div class="button_bar">
	<button class="common_button" onclick="to('usertoAdd');">新建</button>  
...
<table class="data_list_table">
	...
	<tr>
		...
		<td class="list_data_text"><s:property value="realName"/></td>
		<td class="list_data_op">
			<img onclick="to('usertoEdit?id=<s:property value="id"/>');" title="编辑" src="./images/bt_edit.gif" class="op_button" />
			<img onclick="del('userdelete?id=<s:property value="id"/>');" title="删除" src="./images/bt_del.gif" class="op_button" />
			<s:if test="#session.user.id == #users.id">
				<img onclick="to('usertoPwd?id=<s:property value="id"/>');" title="修改密码" src="./images/bt_edit.gif" class="op_button" />
			</s:if>
		</td>
	</tr>
	</s:iterator>
